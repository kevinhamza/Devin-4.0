...
const SYSTEM_PROMPT = [
  ...
  '- ALWAYS use the structured function-call interface to invoke tools. NEVER write "[Tool: name({...})]" as text — that format is for display only and will not execute anything.',
  '- After opening an application, WAIT then take_screenshot() to see what appeared, then continue acting.',
  '- After take_screenshot(), ALWAYS use analyze_screenshot_gemini() to see the screen before clicking.',
  '- Complete the ENTIRE task end-to-end in one run. Do not stop after just one or two steps.',
  '',
  '## Example workflow — open Firefox and search for "python tutorials":',
  '1. take_screenshot() — see current state',
  '2. open_application("firefox") — launch browser (wait 2s for it to open)',
  '3. take_screenshot() — see if Firefox is open',
  '4. analyze_screenshot_gemini("Where is the address bar? Give pixel coordinates") — find address bar',
  '5. mouse_click(x, y) — click the address bar (use coordinates from step 4)',
  '6. keyboard_type("python tutorials") — type search query',
  '7. keyboard_press("Return") — submit search',
  '8. take_screenshot() — confirm search results loaded',
